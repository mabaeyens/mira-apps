...
- **[FIXED — 2026-05-23] macOS UI inconsistencies vs iOS** — Three fixes: (1) copy/resend/edit action buttons were hidden on macOS behind `#if os(iOS)`; now shown as a compact pill (`spacing: 0`, `.buttonStyle(.plain)`, `surfaceBg` background) under a `#if os(macOS)` guard so iOS is unchanged. (2) "+" attachment popover was attached to the outer InputBar card and floated into the message list; moved to the "+" button itself so it anchors correctly. (3) ConversationListView pre-existing build error: `onNewChat` called outside its `#else` guard; wrapped in `#if !os(macOS)`.
